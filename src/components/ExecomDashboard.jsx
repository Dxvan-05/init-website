...
function ExecomDashboard() {
  ...
  const addExecomMember = () => {
    if( !(name && position && description && linkedInUrl && instagramUrl && imageUrl)) {
      alert('A field in empty. Can\'t add Execom Member');
      return;
    }

    axios.post('/execom/create', {
      name,
      position,
      description,
      linkedInUrl,
  ...
  return (
    <div className='w-[80%]'>
      ...
      <div className="my-24 max-w-[1250px] ms-5 grid gap-2 grid-cols-[repeat(auto-fill,350px)] justify-start items-center">
        <ExecomDashboardCard membersList={execomMembers} toggleRefresh={toggleRefresh} />
      </div>
